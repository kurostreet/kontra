/* KONTRA SITE v32 — SOCIAL + PLAYER TRADING MARKET */
(() => {
  "use strict";
  ...
  function renderLvlShop() {
    ...
    root.appendChild(lvlSectionHeader(t("functionalStore"), t("shopNextStage"), `${LVL_SKINS.length} ${t("models")}`));
    const hint = document.createElement("p");
    hint.className = "lvl-data-hint lvl-data-hint--safe-shop";
    hint.textContent = t("functionalStoreHint");
    root.appendChild(hint);
    const bootstrap = lvlReadOnlyData();
    if (!authState.sessionToken) {
      root.appendChild(lvlEmptyState(t("bootstrapLogin"), t("functionalStoreHint")));
      return;
    }
    if (!bootstrap?.sync?.ready) {
    ...
    const owned = lvlOwnedSkinSet(bootstrap);
    const activeId = actualLvlSkinId();
    const busy = Boolean(lvlHubState.skinAction?.busy);
    const grid = document.createElement("div");
    grid.className = "lvl-store-grid";
    for (const skin of LVL_SKINS) {
      const isOwned = owned.has(skin.skinId);
      const isActive = activeId === skin.skinId;
      const isThisBusy = busy && lvlHubState.skinAction.skinId === skin.skinId;
      const isBuyConfirm = !isOwned && skinBuyConfirmed(skin.skinId);
      const row = document.createElement("article");
      row.className = `lvl-store-item${isOwned ? " is-owned" : ""}${isActive ? " is-active" : ""}${isBuyConfirm ? " is-buy-confirm" : ""}`;
      const img = document.createElement("img");
      decorateLvlSkinImage(img, skin);
      img.loading = "lazy";
      img.decoding = "async";
      const copy = document.createElement("div");
      const small = document.createElement("small");
      small.textContent = isActive ? t("active") : (isOwned ? t("owned") : t("price"));
      const strong = document.createElement("strong");
      strong.textContent = skin.name;
      const span = document.createElement("span");
      span.textContent = `${skin.price.toLocaleString(lvlLocale())} ${t("tokens")}`;
      copy.append(small, strong, span);

      const action = document.createElement("button");
      action.type = "button";
      action.className = `lvl-store-buy${isOwned ? " is-owned" : ""}${isActive ? " is-active" : ""}`;
      action.disabled = busy || isActive;
      if (isThisBusy) action.textContent = lvlHubState.skinAction.mode === "buy" ? t("skinBuying") : t("skinApplying");
      else if (isActive) action.textContent = t("active");
      else if (isOwned) action.textContent = t("selectSkinAction");
      else action.textContent = isBuyConfirm ? t("confirmSkinPurchase") : t("buySkinAction");
      action.addEventListener("click", () => {
        if (isOwned) void selectOwnedLvlSkin(skin.skinId);
        else void buyLvlSkin(skin.skinId);
      });
