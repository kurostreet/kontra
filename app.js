...
(() => {
  ...
  function renderLvlShop() {
    ...
    for (const item of cat.items) {
      const isOn = item.owned && lvlEquipmentIsOn(bootstrap, cat.key, item.itemId);
      ...
      const row = document.createElement("article");
      row.className = `lvl-shop-item-v33${item.owned ? " is-owned" : ""}${isOn ? " is-active" : ""}`;
      const visual = lvlItemVisual(cat.key, item.itemId);
      const copy = document.createElement("div"); copy.className = "lvl-shop-item__copy";
      const small = document.createElement("small"); small.textContent = isOn ? lvlLocalized("ВКЛЮЧЕНО", "ENABLED") : (item.owned ? lvlLocalized("ВЫКЛЮЧЕНО", "DISABLED") : lvlLocalized("ЦЕНА", "PRICE"));
      ...
      if (item.owned) {
        ...
        action.disabled = busy || lvlHubState.equipmentAction.busy;
        action.textContent = thisBusy ? lvlLocalized("ПОКУПКА...", "PURCHASING...") : lvlLocalized("КУПИТЬ", "BUY");
