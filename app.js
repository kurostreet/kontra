...
(() => {
  ...
  const LVL_SKINS = Object.freeze([
    { skinId: "sas", name: "SAS", price: 5000 },
    { skinId: "gsg9", name: "GSG 9", price: 5000 },
    { skinId: "gsg9_admin", name: "GSG 9 ADMIN", price: 5000 },
    { skinId: "gign", name: "GIGN", price: 5000 },
    { skinId: "sas_custom", name: "SAS CUSTOM", price: 5000 },
    { skinId: "elite_arctic", name: "ELITE ARCTIC", price: 5000 },
    { skinId: "leet", name: "LEET", price: 5000 },
    { skinId: "leet_pirate", name: "LEET PIRATE", price: 5000 },
    { skinId: "professional", name: "PROFESSIONAL", price: 5000 },
    { skinId: "survivor", name: "SURVIVOR", price: 10000 }
  ]);
  let lvlHubState = {
    tab: "overview",
    previewSkinId: "sas"
  ...
  function currentLvlSkinId() {
    const synced = String(settingsState.data?.desired?.modules?.lvl?.ctSkin || settingsState.data?.snapshot?.modules?.lvl?.values?.ctSkin || "");
    if (LVL_SKINS.some((skin) => skin.skinId === synced)) return synced;
    return lvlHubState.previewSkinId;
  }
  ...
  function renderLvlSkinGallery() {
    ...
    if (!gallery) return;
    const activeId = currentLvlSkinId();
    gallery.replaceChildren();
    LVL_SKINS.forEach((skin, index) => {
      const card = document.createElement("button");
      card.type = "button";
      card.className = `skin-card${skin.skinId === activeId ? " is-active" : ""}`;
      card.dataset.skinPreview = skin.skinId;
      const demoOwned = index < 3 || skin.skinId === activeId;
      card.innerHTML = `
        <span class="skin-card__visual"><img src="assets/skins/skin-demo.webp" alt="" width="160" height="200"><i>${String(index + 1).padStart(2, "0")}</i></span>
        <span class="skin-card__copy"><small>${skin.skinId === activeId ? t("active") : (demoOwned ? t("owned") : t("locked"))}</small><strong>${skin.name}</strong><em>${t("price")}: ${skin.price.toLocaleString(language === "ru" ? "ru-RU" : "en-US")} ${t("tokens")}</em></span>
        <b>${t("preview")}</b>`;
  ...
  function renderLvlHub() {
    const skin = lvlSkinById(currentLvlSkinId());
    $$('[data-lvl-active-name]').forEach((node) => { node.textContent = skin.name; });
    $$('[data-lvl-active-image]').forEach((image) => { image.src = "assets/skins/skin-demo.webp"; });
    ...
    if (name) name.textContent = profile?.name || account?.username || (language === "ru" ? "ГОСТЬ" : "GUEST");
    if (avatar) avatar.src = avatarSource(profile?.avatarId || DEFAULT_AVATAR_ID);
    if (level) level.textContent = profile ? `LVL ${profile.level}` : "LVL —";
    if (tokens) tokens.textContent = profile ? Number(profile.tokens || 0).toLocaleString(language === "ru" ? "ru-RU" : "en-US") : "—";
