/* KONTRA SITE v37 — UPGRADES + WEAPON CRATE + OVERVIEW NAV */
(() => {
  "use strict";
  ...
  const LVL_SHOP_FALLBACK = Object.freeze([
    { key:"upgrades", ru:"ПРОКАЧКА", en:"UPGRADES", items:[
      ["lvl10","+10 LVL","+10 LVL",1000],
      ["lvl_x2","LVL ×2 · 3 часа","LVL ×2 · 3 hours",1500],
      ["tok_x2","ТОКЕНЫ ×2 · 3 часа","TOKENS ×2 · 3 hours",250]
    ] },
    { key:"crate", ru:"ЯЩИК С ОРУЖИЕМ", en:"WEAPON CRATE", items:[
      ["gold","Золотой AK47 + Deagle · 30 мин","Golden AK47 + Deagle · 30 min",300],
      ["minigun","Миниган + Базука · 30 мин","Minigun + Bazooka · 30 min",500],
      ["armor50","50 брони каждый раунд · 30 мин","50 armor every round · 30 min",50],
      ["armor100","100 брони каждый раунд · 30 мин","100 armor every round · 30 min",100],
      ["invis","Авто-инвиз КТ · 3 заряда","CT auto-invis · 3 charges",150],
      ["blind","Слепой нож КТ · 30 мин","CT blind knife · 30 min",200],
      ["gas","Противогаз КТ · 30 мин","CT gas mask · 30 min",80]
    ] },
      ...
      return {
        ...
        items: cat.items.map(([itemId,itemRu,itemEn,price]) => {
          ...
          return {
            ...
            owned:optionKey ? owned.has(id) : false, active:optionKey ? activeSet.has(id) : (remainingSec>0 || charges>0),
  ...
  function lvlCategoryIcon(category) {
    return ({ upgrades: "↑", crate: "✚", prefix: "TAG", prefcolor: "◐", skin: "◉", skincolor: "◈", crit: "✦", minimap: "▦", wsound: "♪" })[String(category || "")] || "◇";
  }
  ...
  const LVL_MANAGED_FIELDS = new Set(["prefix", "prefixColor", "ctSkin", "skinColor", "critBullet", "minimap", "weaponSounds"]);
  ...
  function lvlEquipmentIsOn(bootstrap, categoryOrField, itemId) {
    ...
    if (field === "weaponSounds") return new Set(Array.isArray(value) ? value.map(String) : []).has(id);
  ...
  async function setLvlEquipmentState(categoryOrField, itemId, enabled) {
    ...
    try {
      const saved = await settingsRequest("/me", {
        method: "POST",
        body: { desired: { modules: { lvl: { [field]: value } } } }
      });

      if (lvlHubState.bootstrap?.settings) {
        lvlHubState.bootstrap.settings.desired = { ...(lvlHubState.bootstrap.settings.desired || {}), [field]: value };
