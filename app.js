...
(() => {
  ...
  let lvlHubState = {
    ...
    loadedAt: 0,
    error: "",
    skinAction: { busy: false, skinId: "", error: "" }
  };
  ...
  function renderLvlSkinGallery() {
    ...
    LVL_SKINS.forEach((skin, index) => {
      ...
      card.type = "button";
      card.className = `skin-card${isActive ? " is-active" : ""}${isPreview ? " is-previewing" : ""}${isPending ? " is-pending" : ""}${isApplying ? " is-applying" : ""}`;
      ...
      action.textContent = isApplying ? t("skinApplying") : (isActive ? t("active") : (isPending ? t("skinPending") : (isOwned ? t("selectSkinAction") : t("preview"))));
