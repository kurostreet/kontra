...
(() => {
  ...
  let lvlHubState = {
    ...
    socialAction: { busy: false, key: "", error: "" },
  ...
  async function runLvlShopAction(category, itemId) {
    ...
    try {
      ...
      const map = {
        player_offline: lvlLocalized("Для покупки зайдите на игровой сервер", "Join the game server to purchase"),
  ...
  function renderLvlShop() {
    ...
    const bootstrap = lvlReadOnlyData();
    const categories = Array.isArray(bootstrap?.shop?.categories) ? bootstrap.shop.categories.filter((cat) => cat?.enabled !== false && Array.isArray(cat?.items) && cat.items.length) : [];
  ...
  function renderLvlMarket() {
    ...
    const sellable = Array.isArray(market.sellable) ? market.sellable : [];
    const totalListings = Math.max(listings.length, Number(market.totalListings || listings.length));
    const rules = market.rules || {};
    const maxListings = Number(rules.maxListings || 0);
    const marketDays = Number(rules.marketDays || 0);
    const taxPct = Number(rules.taxPct || 0);
  ...
  async function runLvlSocialAction(action, payload = {}) {
    ...
    if (!authState.sessionToken) { setLogin(true); return false; }
    const previousSyncedAt = Number(lvlReadOnlyData()?.sync?.syncedAt || 0);
    lvlHubState.socialAction = { busy: true, key: action, error: "" };
    renderLvlFriends();
    try {
      const created = await controlRequest("/command", { method: "POST", body: { action, ...payload } });
      const commandId = String(created?.commandId || "");
      ...
      toast(lvlLocalized("Выполнено", "Done"));
      return true;
    } catch (error) {
