/* KONTRA SITE v29 — WORKING OWNED SKIN SELECT / LVL HUB */
(() => {
  "use strict";
  ...
  function setLvlHubTab(tab) {
    const allowed = new Set(["overview", "skins", "shop", "inventory", "friends", "battlepass", "settings"]);
  ...
  function renderLvlFriends() {
    ...
    root.appendChild(lvlSectionHeader(t("hubFriends"), t("friendsTitle"), social ? `${social.counts?.friends || 0}` : ""));
    if (!authState.sessionToken) {
      root.appendChild(lvlEmptyState(t("bootstrapLogin"), t("readOnlyHint")));
      return;
    }
    ...
    const lists = [["requests", t("friendRequests")], ["friends", t("friendsList")], ["followers", t("followers")], ["subscriptions", t("subscriptions")]];
    for (const [key, label] of lists) {
      ...
      const chips = document.createElement("div"); chips.className = "lvl-data-chips";
      if (values.length) values.forEach((item) => chips.appendChild(lvlChip(String(item?.name || "Player"))));
      else chips.appendChild(lvlChip(t("noFriends")));
      section.append(title, chips); root.appendChild(section);
    }
    ...
    for (const message of inbox) {
      ...
      item.className = `lvl-message${message?.read === false ? " is-unread" : ""}`;
      ...
      item.append(head, p); messages.appendChild(item);
    }
    section.append(title, messages); root.appendChild(section);
  }
