/* KONTRA SITE v33 — SHOP CATEGORIES + OFFLINE MESSAGES + MARKET */
(() => {
  "use strict";
  ...
  const LVL_SKINS = Object.freeze([
    { skinId: "sas", name: "SAS", price: 5000, preview: "assets/skins/sas.webp" },
    { skinId: "gsg9", name: "GSG 9", price: 5000, preview: "assets/skins/gsg9.webp" },
    { skinId: "gsg9_admin", name: "GSG 9 ADMIN", price: 5000, preview: "assets/skins/gsg9_admin.webp" },
    { skinId: "gign", name: "GIGN", price: 5000, preview: "assets/skins/gign.webp" },
    { skinId: "sas_custom", name: "SAS CUSTOM", price: 5000, preview: "assets/skins/sas_custom.webp" },
    { skinId: "elite_arctic", name: "ELITE ARCTIC", price: 5000, preview: "assets/skins/elite_arctic.webp" },
    { skinId: "leet", name: "LEET", price: 5000, preview: "assets/skins/leet.webp" },
    { skinId: "leet_pirate", name: "LEET PIRATE", price: 5000, preview: "assets/skins/leet_pirate.webp" },
    { skinId: "professional", name: "PROFESSIONAL", price: 5000, preview: "assets/skins/professional.webp" },
    { skinId: "survivor", name: "SURVIVOR", price: 10000, preview: "assets/skins/survivor.webp" }
  ]);
  const LVL_SHOP_FALLBACK = Object.freeze([
    { key:"prefix", ru:"ПРЕФИКСЫ", en:"PREFIXES", items:[
      ["noob","НУБ","NOOB",2500],["bot","БОТ","BOT",2500],["pro","ПРО","PRO",2500],["king","КИНГ","KING",2500],
      ["boss","БОСС","BOSS",2500],["legend","ЛЕГЕНДА","LEGEND",2500],["master","МАСТЕР","MASTER",2500],["guru","ГУРУ","GURU",2500],
      ["titan","ТИТАН","TITAN",2500],["lord","ЛОРД","LORD",2500],["demon","ДЕМОН","DEMON",2500],["warrior","ВОИН","WARRIOR",2500]
    ] },
    ...
    { key:"skincolor", ru:"ЦВЕТ СКИНА КТ", en:"CT SKIN COLOR", items:[
      ["#0044FF","Синий","Blue",2500],["#0000FF","Тёмно-синий","Navy",2500],["#00AAFF","Голубой","LtBlue",2500],["#00FFFF","Морской","Cyan",2500],
      ["#00FFAA","Бирюзовый","Teal",2500],["#00CC00","Зелёный","Green",2500],["#00FF00","Лайм","Lime",2500],["#FFFF00","Жёлтый","Yellow",2500],
      ["#FFD700","Золотой","Gold",2500],["#FFFFFF","Белый","White",2500]
    ] },
    { key:"crit", ru:"КРИТИЧЕСКИЙ ВЫСТРЕЛ", en:"CRITICAL SHOT", items:[
      ["fire","Зажигательная пуля","Incendiary bullet",5300],["freeze","Замораживающая пуля","Freezing bullet",5600],["dmg2","Урон Х2 пуля","Damage X2 bullet",5400]
    ] },
    { key:"minimap", ru:"МИНИ-КАРТА", en:"MINIMAP", items:[["minimap","Мини-карта","Minimap",7000]] },
    { key:"wsound", ru:"ЗВУКИ ОРУЖИЯ", en:"WEAPON SOUNDS", items:[
      ["gold_ak_guitar","Золотой АК47 — Гитара","Golden AK47 — Guitar",6500],["minigun_mg","Миниган","Minigun",6500],
      ["gold_deagle_laser","Золотой Дигл — Лазер","Golden Deagle — Laser",6500],["knife_starwars","Нож — Star Wars","Knife — Star Wars",6500]
    ] }
  ]);

  const LVL_SHOP_OPTION_KEYS = Object.freeze({
    prefix:"prefix", prefcolor:"prefixColor", skin:"ctSkin", skincolor:"skinColor", crit:"critBullet", minimap:"minimap", wsound:"weaponSounds"
  });
    ...
    return LVL_SHOP_FALLBACK.map((cat) => {
      ...
      return {
        ...
        items: cat.items.map(([itemId,itemRu,itemEn,price]) => ({
          itemId:String(itemId), itemRu:String(itemRu), itemEn:String(itemEn), price:Number(price)||0,
          owned:owned.has(String(itemId)), active:activeSet.has(String(itemId))
        }))
      };
    });
  }
  ...
  function lvlFallbackMarketSellable(bootstrap, categories) {
    ...
    for (const cat of categories) {
      for (const item of (Array.isArray(cat?.items) ? cat.items : [])) {
        ...
        const key=`${String(cat.key||"")}|${String(item.itemId||"")}`;
        if (mine.has(key)) continue;
        const maxPrice=Math.max(0,Math.floor(Number(item.price)||0));
        if (!maxPrice) continue;
        out.push({ category:cat.key, categoryRu:cat.ru, categoryEn:cat.en, itemId:item.itemId, itemRu:item.itemRu, itemEn:item.itemEn, maxPrice, active:item.active===true });
      }
    }
    return out;
  }
  ...
  function renderPersistentPanel(panelName, data) {
    ...
    const desired = data.desired?.modules?.lvl || moduleData?.values || {};
    if (!moduleData) {
      appendSettingsState(actions, t("settingsNotSynced"), t("settingsNotSyncedText"));
      setControlStatus(t("settingsNotSynced"), "error");
      return;
  ...
  function lvlCategoryIcon(category) {
    return ({ prefix: "TAG", prefcolor: "◐", skin: "◉", skincolor: "◈", crit: "✦", minimap: "▦", wsound: "♪" })[String(category || "")] || "◇";
  }
  ...
  function actualLvlSkinId() {
    const bootstrapId = String(lvlReadOnlyData()?.skin?.activeId || "");
    if (LVL_SKINS.some((skin) => skin.skinId === bootstrapId)) return bootstrapId;
    const synced = String(settingsState.data?.snapshot?.modules?.lvl?.values?.ctSkin || "");
    if (LVL_SKINS.some((skin) => skin.skinId === synced)) return synced;
    return "sas";
  }

  function previewLvlSkinId() {
    return LVL_SKINS.some((skin) => skin.skinId === lvlHubState.previewSkinId) ? lvlHubState.previewSkinId : actualLvlSkinId();
  }
  ...
  function setLvlHubTab(tab) {
    const allowed = new Set(["overview", "skins", "shop", "market", "inventory", "friends", "battlepass", "settings"]);
    lvlHubState.tab = allowed.has(tab) ? tab : "overview";
    $$('[data-lvl-tab]').forEach((button) => button.classList.toggle("is-active", button.dataset.lvlTab === lvlHubState.tab));
  ...
  async function buyLvlSkin(skinId) {
    ...
    try {
      ...
      } else if (message.includes("command_rate_limited")) {
        toast(t("controlRateLimited"));
      } else {
        toast(t("skinBuyFailed"));
      }
      await fetchLvlBootstrap(true);
  ...
  function renderLvlSkinGallery() {
    ...
    const owned = lvlOwnedSkinSet(bootstrap);
    const actionBusy = Boolean(lvlHubState.skinAction?.busy);
    gallery.replaceChildren();
    LVL_SKINS.forEach((skin, index) => {
      const isActive = skin.skinId === actualId;
      ...
      const isApplying = actionBusy && lvlHubState.skinAction.skinId === skin.skinId;
      const isBuying = isApplying && lvlHubState.skinAction.mode === "buy";
      ...
      action.textContent = isBuying ? t("skinBuying") : (isApplying ? t("skinApplying") : (isActive ? t("active") : (isPending ? t("skinPending") : (isOwned ? t("selectSkinAction") : (isBuyConfirm ? `${t("confirmSkinPurchase")} · ${skin.price.toLocaleString(lvlLocale())}` : `${t("buySkinAction")} · ${skin.price.toLocaleString(lvlLocale())}`)))));
  ...
  function renderLvlShop() {
    ...
    for (const item of cat.items) {
      const row = document.createElement("article");
      row.className = `lvl-shop-item-v33${item.owned ? " is-owned" : ""}${item.active ? " is-active" : ""}`;
      const visual = lvlItemVisual(cat.key, item.itemId);
      const copy = document.createElement("div"); copy.className = "lvl-shop-item__copy";
      const small = document.createElement("small"); small.textContent = item.active ? lvlLocalized("АКТИВЕН", "ACTIVE") : (item.owned ? lvlLocalized("КУПЛЕН", "OWNED") : lvlLocalized("ЦЕНА", "PRICE"));
      const strong = document.createElement("strong"); strong.textContent = lvlLocalized(item.itemRu, item.itemEn, item.itemId);
      const price = document.createElement("span"); price.textContent = `${Number(item.price || 0).toLocaleString(lvlLocale())} ${t("tokens")}`;
      copy.append(small, strong, price);
      const action = document.createElement("button"); action.type = "button"; action.className = "lvl-store-buy";
      const thisBusy = busy && lvlHubState.shopAction.key === `${cat.key}|${item.itemId}`;
      action.disabled = busy || item.owned || item.active;
      action.textContent = thisBusy ? lvlLocalized("ПОКУПКА...", "PURCHASING...") : (item.active ? lvlLocalized("АКТИВЕН", "ACTIVE") : item.owned ? lvlLocalized("КУПЛЕН", "OWNED") : lvlLocalized("КУПИТЬ", "BUY"));
      action.addEventListener("click", async () => {
        ...
        await runLvlShopAction(cat.key, String(item.itemId || ""));
      });
  ...
  async function runLvlMarketAction(action, payload = {}) {
    ...
    try {
      ...
      const map = {
        player_offline: lvlLocalized("Зайдите на игровой сервер для операции", "Join the game server to perform this action"),
  ...
  function renderLvlInventory() {
    ...
    const defs = [
      ...
      ["weaponSounds", t("weaponSoundsSetting")]
    ];
    let shown = 0;
    for (const [key, label] of defs) {
      ...
      shown += list.length;
      const section = document.createElement("section");
      section.className = "lvl-data-group";
      const title = document.createElement("h3");
      title.textContent = `${label} · ${list.length}`;
      const chips = document.createElement("div");
      chips.className = "lvl-data-chips";
      for (const item of list) chips.appendChild(lvlChip(lvlLocalized(item?.ru, item?.en, item?.id), String(item?.id || "")));
  ...
  function renderLvlFriends() {
    ...
    tokenNote.textContent = lvlLocalized(
      `Доступно сегодня: ${Number(transfer.leftToday || 0).toLocaleString(lvlLocale())} / ${Number(transfer.dailyLimit || 0).toLocaleString(lvlLocale())} · с LVL ${transfer.minLevel || 0} · получатель должен быть в игре`,
      `Today left: ${Number(transfer.leftToday || 0).toLocaleString(lvlLocale())} / ${Number(transfer.dailyLimit || 0).toLocaleString(lvlLocale())} · from LVL ${transfer.minLevel || 0} · recipient must be online`
    );
  ...
  async function runLvlSocialAction(action, payload = {}) {
    ...
    try {
      ...
      const map = {
        target_offline: lvlLocalized("Игрок сейчас не в игре", "Player is offline"),
  ...
  function renderLvlSettingsSummary() {
    ...
    const defs = [
      ...
      ["skinColor", t("skinColorSetting")], ["critBullet", t("critBulletSetting")], ["minimap", t("minimapSetting")], ["weaponSounds", t("weaponSoundsSetting")]
    ];
    ...
    for (const [key, label] of defs) {
      ...
      const small = document.createElement("small"); small.textContent = label;
      const strong = document.createElement("strong");
      strong.textContent = key === "language" ? String(actual[key] || "—").toUpperCase() : lvlSettingOptionLabel(key, actual[key], bootstrap);
      copy.append(small, strong);
      ...
      if (a !== d) {
        ...
        pending.textContent = `${t("pendingValue")}: ${key === "language" ? String(desired[key] || "—").toUpperCase() : lvlSettingOptionLabel(key, desired[key], bootstrap)}`;
      } else {
  ...
  function renderLvlHub() {
    const bootstrap = lvlReadOnlyData();
    const actualSkin = lvlSkinById(actualLvlSkinId());
    $$('[data-lvl-actual-name]').forEach((node) => { node.textContent = actualSkin.name; });
    $$('[data-lvl-actual-image]').forEach((image) => { decorateLvlSkinImage(image, actualSkin); });
    ...
    if (tokens) tokens.textContent = profile ? Number(profile.tokens || 0).toLocaleString(lvlLocale()) : "—";
    if (!lvlHubState.previewSkinId) lvlHubState.previewSkinId = actualSkin.skinId;
