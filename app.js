/* KONTRA SITE v40 — LVL MOD v89 + email/Google account linking */
(() => {
  "use strict";
  ...
  async function renderGoogleButtons() {
    ...
    try {
      ...
      slots.forEach((slot) => {
        ...
        api.renderButton(slot, { theme: colorTheme === "day" ? "outline" : "filled_black", size: "large", shape: "pill", width: Math.min(360, Math.max(240, slot.clientWidth || 300)) });
      });
    } catch (error) {
  ...
  function createSecurityCenter() {
    ...
    const googleCard=document.createElement("article"); googleCard.className="security-method";
    ...
    const googleHint=document.createElement("small");googleHint.className="security-hint";googleHint.textContent=t("googleLinkHint");
    googleCard.append(googleTop,googleCurrent,googleSlot,googleHint);
    if(security.googleLinked){const unlink=document.createElement("button");unlink.type="button";unlink.className="text-action danger-text";unlink.textContent=t("unlink");unlink.addEventListener("click",async()=>{if(!confirm(authLocalized("Отвязать Google? Игровой логин и пароль останутся рабочими.","Unlink Google? Game username/password will still work.")))return;try{const data=await authRequest("/google/unlink",{method:"POST",body:{}});authState.security=data.security||{};renderAuth();}catch(error){toast(authErrorMessage(error));}});googleCard.append(unlink);}
  ...
  function bind() {
    ...
    $("[data-toggle-email-login]")?.addEventListener("click", () => {
      const panel=$("#emailLoginPanel"); if(!panel) return; panel.hidden=!panel.hidden; if(!panel.hidden) $("#emailLoginIdentifier")?.focus();
    });
