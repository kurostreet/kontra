/* KONTRA SITE v26 — LVL HUB UI PREVIEW / NIGHT DEFAULT */
(() => {
  "use strict";
  ...
  const i18n = {
    en: {
      ...
      active: "ACTIVE",
      price: "PRICE",
      skinUiDemoNote: "This build is a visual prototype. Selecting a card changes only the preview; Worker and Lua are untouched.",
      ...
      skinReadOnly: "Actual game selection. Gallery clicks are preview-only.",
      skinReadOnlyNote: "Ownership and active skin come from /lvl/bootstrap. Card clicks do not change the game yet.",
    ...
    ru: {
      ...
      active: "АКТИВЕН",
      price: "ЦЕНА",
      skinUiDemoNote: "Это визуальный прототип. Нажатие на карточку меняет только предпросмотр; Worker и Lua не трогаются.",
      ...
      skinReadOnly: "Показывается реальный выбор в игре. Карточки ниже — только предпросмотр.",
      skinReadOnlyNote: "Купленные и активный скин приходят из /lvl/bootstrap. Нажатие на карточку пока не меняет игру.",
  ...
  function renderLvlSkinGallery() {
    ...
    const owned = new Set(Array.isArray(bootstrap?.skin?.ownedIds) ? bootstrap.skin.ownedIds.map(String) : []);
    gallery.replaceChildren();
    LVL_SKINS.forEach((skin, index) => {
      const isActive = skin.skinId === actualId;
      const isPreview = skin.skinId === previewId && !isActive;
      const isOwned = owned.has(skin.skinId) || isActive;
      const card = document.createElement("button");
      card.type = "button";
      card.className = `skin-card${isActive ? " is-active" : ""}${isPreview ? " is-previewing" : ""}`;
      ...
      const status = document.createElement("small");
      status.textContent = isActive ? t("active") : (isOwned ? t("owned") : t("locked"));
      const name = document.createElement("strong");
  ...
  function renderLvlShop() {
    ...
    root.appendChild(lvlSectionHeader(t("readOnlyStore"), t("shopNextStage"), `${LVL_SKINS.length} ${t("models")}`));
    const hint = document.createElement("p");
    hint.className = "lvl-data-hint";
    hint.textContent = t("readOnlyHint");
    ...
    for (const skin of LVL_SKINS) {
      const row = document.createElement("article");
      row.className = `lvl-store-item${owned.has(skin.skinId) || actualLvlSkinId() === skin.skinId ? " is-owned" : ""}`;
      const img = document.createElement("img");
      decorateLvlSkinImage(img, skin);
      img.loading = "lazy";
      img.decoding = "async";
      const copy = document.createElement("div");
      const small = document.createElement("small");
      small.textContent = owned.has(skin.skinId) || actualLvlSkinId() === skin.skinId ? t("owned") : t("price");
      const strong = document.createElement("strong");
